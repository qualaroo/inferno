...
function documentCreateElement(tag, isSVG, doc) {
    if (isSVG) {
...
function getDocumentId(doc) {
    for (var docId in documentsScopes) {
...
var uniqueId = 1;
function registerDocumentScope(doc) {
    var docId = uniqueId++;
...
function handleEvent(name, nextEvent, dom, doc) {
...
function attachEventToDocument(name, doc) {
...
function remove(vNode, parentDOM, doc) {
    unmount(vNode, doc);
    if (parentDOM) {
        removeVNodeDOM(vNode, parentDOM);
    }
}
function unmount(vNode, doc) {
    var flags = vNode.flags;
    var children = vNode.children;
    var ref;
    if (flags & 481 /* Element */) {
...
function unmountAllChildren(children, doc) {
    for (var i = 0, len = children.length; i < len; ++i) {
...
function removeAllChildren(dom, vNode, children, doc) {
    unmountAllChildren(children, doc);
...
function patchProp(prop, lastValue, nextValue, dom, isSVG, hasControlledValue, lastVNode, doc) {
    switch (prop) {
        case 'children':
        case 'childrenType':
        case 'className':
                ...
                if (!isNullOrUndef(nextHtml) && !isSameInnerHTML(dom, nextHtml)) {
                    if (!isNull(lastVNode)) {
                        if (lastVNode.childFlags & 12 /* MultipleChildren */) {
                            unmountAllChildren(lastVNode.children, doc);
                        }
...
function mountProps(vNode, flags, props, dom, isSVG, doc) {
...
function mount(vNode, parentDOM, context, isSVG, nextNode, lifecycle, doc) {
...
function mountPortal(vNode, context, parentDOM, nextNode, lifecycle, doc) {
    mount(vNode.children, vNode.ref, context, false, null, lifecycle, doc);
    var placeHolderVNode = createVoidVNode();
    mountText(placeHolderVNode, parentDOM, nextNode, doc);
    vNode.dom = placeHolderVNode.dom;
}
function mountFragment(vNode, parentDOM, context, isSVG, nextNode, lifecycle, doc) {
    var children = vNode.children;
    var childFlags = vNode.childFlags;
    ...
    if (childFlags === 2 /* HasVNodeChildren */) {
        mount(children, parentDOM, nextNode, isSVG, nextNode, lifecycle, doc);
    }
    else {
        mountArrayChildren(children, parentDOM, context, isSVG, nextNode, lifecycle, doc);
    }
}
function mountText(vNode, parentDOM, nextNode, doc) {
    var dom = (vNode.dom = doc.createTextNode(vNode.children));
    if (!isNull(parentDOM)) {
        insertOrAppend(parentDOM, dom, nextNode);
    }
}
function mountElement(vNode, parentDOM, context, isSVG, nextNode, lifecycle, doc) {
    var flags = vNode.flags;
    var props = vNode.props;
    ...
    var dom = documentCreateElement(vNode.type, isSVG, doc);
    vNode.dom = dom;
    ...
    if (childFlags === 16 /* HasTextChildren */) {
        ...
        if (childFlags === 2 /* HasVNodeChildren */) {
            ...
            mount(children, dom, context, childrenIsSVG, null, lifecycle, doc);
        }
...
function mountArrayChildren(children, dom, context, isSVG, nextNode, lifecycle, doc) {
    for (var i = 0, len = children.length; i < len; ++i) {
...
function mountClassComponent(vNode, parentDOM, context, isSVG, nextNode, lifecycle, doc) {
    var instance = createClassComponentInstance(vNode, vNode.type, vNode.props || EMPTY_OBJ, context, isSVG, lifecycle);
    mount(instance.$LI, parentDOM, instance.$CX, isSVG, nextNode, lifecycle, doc);
    mountClassComponentCallbacks(vNode.ref, instance, lifecycle);
}
function mountFunctionalComponent(vNode, parentDOM, context, isSVG, nextNode, lifecycle, doc) {
    var type = vNode.type;
    var props = vNode.props || EMPTY_OBJ;
    var ref = vNode.ref;
...
function replaceWithNewNode(lastVNode, nextVNode, parentDOM, context, isSVG, lifecycle, doc) {
...
function patch(lastVNode, nextVNode, parentDOM, context, isSVG, nextNode, lifecycle, doc) {
...
function patchFragment(lastVNode, nextVNode, parentDOM, context, isSVG, lifecycle, doc) {
...
function patchPortal(lastVNode, nextVNode, context, lifecycle, doc) {
    var lastContainer = lastVNode.ref;
    var nextContainer = nextVNode.ref;
    var nextChildren = nextVNode.children;
    patchChildren(lastVNode.childFlags, nextVNode.childFlags, lastVNode.children, nextChildren, lastContainer, context, false, null, lastVNode, lifecycle, doc);
    nextVNode.dom = lastVNode.dom;
...
function patchElement(lastVNode, nextVNode, context, isSVG, nextFlags, lifecycle, doc) {
        ...
        if (lastPropsOrEmpty !== EMPTY_OBJ) {
            for (var prop$1 in lastPropsOrEmpty) {
                if (isNullOrUndef(nextPropsOrEmpty[prop$1]) && !isNullOrUndef(lastPropsOrEmpty[prop$1])) {
                    patchProp(prop$1, lastPropsOrEmpty[prop$1], null, dom, isSVG, hasControlledValue, lastVNode, doc);
                }
            }
        }
    }
    ...
    if (nextFlags & 4096 /* ContentEditable */) {
        ...
        patchChildren(lastVNode.childFlags, nextVNode.childFlags, lastVNode.children, nextChildren, dom, context, isSVG && nextVNode.type !== 'foreignObject', null, lastVNode, lifecycle, doc);
    }
    if (isFormElement) {
...
function replaceOneVNodeWithMultipleVNodes(lastChildren, nextChildren, parentDOM, context, isSVG, lifecycle, doc) {
...
function patchChildren(lastChildFlags, nextChildFlags, lastChildren, nextChildren, parentDOM, context, isSVG, nextNode, parentVNode, lifecycle, doc) {
...
function updateClassComponent(instance, nextState, nextProps, parentDOM, context, isSVG, force, nextNode, lifecycle, doc) {
...
function patchClassComponent(lastVNode, nextVNode, parentDOM, context, isSVG, nextNode, lifecycle, doc) {
...
function patchFunctionalComponent(lastVNode, nextVNode, parentDOM, context, isSVG, nextNode, lifecycle, doc) {
...
function patchNonKeyedChildren(lastChildren, nextChildren, dom, context, isSVG, lastChildrenLength, nextChildrenLength, nextNode, lifecycle, doc) {
...
function patchKeyedChildren(a, b, dom, context, isSVG, aLength, bLength, outerEdge, parentVNode, lifecycle, doc) {
    var aEnd = aLength - 1;
    var bEnd = bLength - 1;
    var i = 0;
    var j = 0;
...
function __render(input, parentDOM, callback, context) {
    ...
    if (isNullOrUndef(rootInput)) {
        if (!isNullOrUndef(input)) {
            ...
            mount(input, parentDOM, context, false, null, lifecycle, doc);
            parentDOM.$V = input;
            rootInput = input;
        }
    }
        ...
        if (isNullOrUndef(input)) {
            remove(rootInput, parentDOM, doc);
            parentDOM.$V = null;
        }
        else {
            if (input.flags & 16384 /* InUse */) {
            ...
            patch(rootInput, input, parentDOM, context, false, null, lifecycle, doc);
...
function applyState(component, force, callback) {
    ...
    if (force || !component.$BR) {
        ...
        var lifecycle = [];
        renderCheck.v = true;
        var parentDOM = findDOMfromVNode(component.$LI, true).parentNode;
        var doc = (parentDOM || document).ownerDocument || document;
        updateClassComponent(component, combineFrom(component.state, pendingState), component.props, parentDOM, component.context, component.$SVG, force, null, lifecycle, doc);
        if (lifecycle.length > 0) {
            callAll(lifecycle);
        }
        renderCheck.v = false;
...
export { Component, Fragment, EMPTY_OBJ, createComponentVNode, createFragment, createPortal, createRef, createRenderer, createTextVNode, createVNode, forwardRef, directClone, findDOMfromVNode, getFlagsForElementVnode, linkEvent, normalizeProps, options, render, rerender, version, createClassComponentInstance as _CI, handleComponentInput as _HI, mount as _M, mountClassComponentCallbacks as _MCCC, mountElement as _ME, mountFunctionalComponentCallbacks as _MFCC, mountRef as _MR, mountText as _MT, mountProps as _MP, __render };
